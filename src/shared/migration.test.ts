...
describe("migrateAgentNames", () => {
  test("migrates legacy OmO names to lowercase", () => {
    ...
    expect(migrated["architect"]).toEqual({ temperature: 0.5 })
    expect(migrated["product-manager"]).toEqual({ prompt: "custom prompt" })
  ...
  test("preserves current agent names unchanged", () => {
    ...
    expect(migrated["strategist"]).toEqual({ model: "openai/gpt-5.4" })
    expect(migrated["librarian"]).toEqual({ model: "google/gemini-3-flash" })
    expect(migrated["analyst"]).toEqual({ model: "opencode/gpt-5-nano" })
  })
  ...
  test("handles case-insensitive migration", () => {
    ...
    const agents = {
      ...
      "planner-architect": { prompt: "test" },
      "Orchestrator-Architect": { model: "openai/gpt-5.4" },
    }
    ...
    expect(migrated["product-manager"]).toEqual({ prompt: "test" })
    expect(migrated["technical-lead"]).toEqual({ model: "openai/gpt-5.4" })
  })
  ...
  test("migrates orchestrator-architect to technical-lead", () => {
    // given: Config with legacy orchestrator-architect agent name
    const agents = {
      "orchestrator-architect": { model: "anthropic/claude-opus-4-6" },
    }

    // when: Migrate agent names
    const { migrated, changed } = migrateAgentNames(agents)

    // then: orchestrator-architect should be migrated to technicalLead
    expect(changed).toBe(true)
    expect(migrated["technical-lead"]).toEqual({ model: "anthropic/claude-opus-4-6" })
    expect(migrated["orchestrator-architect"]).toBeUndefined()
  })

  test("migrates lowercase technical-lead to technical-lead", () => {
    // given: Config with lowercase technicalLead agent name
    const agents = {
    ...
    // then: lowercase technicalLead should remain technicalLead (no change needed)
    expect(changed).toBe(false)
    expect(migrated["technical-lead"]).toEqual({ model: "anthropic/claude-opus-4-6" })
  })

  test("migrates Architect variants to lowercase", () => {
    ...
    expect(changed).toBe(true)
    expect(migrated["architect"]).toEqual({ model: "test" })
    expect(migrated["Architect"]).toBeUndefined()
  })

  test("migrates omo key to architect", () => {
  ...
  test("migrates TechnicalLead variants to lowercase", () => {
    // given agents config with "TechnicalLead" key
    // when migrateAgentNames called
    // then key becomes "technical-lead"
    const agents = { "TechnicalLead": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["technical-lead"]).toEqual({ model: "test" })
    expect(migrated["TechnicalLead"]).toBeUndefined()
  })

  test("migrates ProductManager variants to lowercase", () => {
    // given agents config with "ProductManager - Plan Builder" key
    // when migrateAgentNames called
    // then key becomes "product-manager"
    const agents = { "ProductManager - Plan Builder": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["product-manager"]).toEqual({ model: "test" })
    expect(migrated["ProductManager - Plan Builder"]).toBeUndefined()
  })

  test("migrates Consultant variants to lowercase", () => {
    // given agents config with "Consultant - Plan Consultant" key
    // when migrateAgentNames called
    // then key becomes "consultant"
    const agents = { "Consultant - Plan Consultant": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["consultant"]).toEqual({ model: "test" })
    expect(migrated["Consultant - Plan Consultant"]).toBeUndefined()
  })

  test("migrates QaEngineer variants to lowercase", () => {
    // given agents config with "QaEngineer - Plan Critic" key
    // when migrateAgentNames called
    // then key becomes "qa-engineer"
    const agents = { "QaEngineer - Plan Critic": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["qa-engineer"]).toEqual({ model: "test" })
    expect(migrated["QaEngineer - Plan Critic"]).toBeUndefined()
  })

  test("migrates Architect-Junior to lowercase", () => {
    // given agents config with "Architect-Junior" key
    // when migrateAgentNames called
    // then key becomes "junior-architect"
    const agents = { "Architect-Junior": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["junior-architect"]).toEqual({ model: "test" })
    expect(migrated["Architect-Junior"]).toBeUndefined()
  })

  test("preserves lowercase passthrough", () => {
    // given agents config with "strategist" key
    // when migrateAgentNames called
    // then key remains "strategist" (no change needed)
    const agents = { "strategist": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(false)
...
describe("migrateHookNames", () => {
  ...
  test("migrates architect-orchestrator to technical-lead", () => {
    // given: Config with legacy architect-orchestrator hook
    const hooks = ["architect-orchestrator", "comment-checker"]

    // when: Migrate hook names
    const { migrated, changed, removed } = migrateHookNames(hooks)

    // then: architect-orchestrator should be migrated to technicalLead
    expect(changed).toBe(true)
    expect(migrated).toContain("technical-lead")
    expect(migrated).toContain("comment-checker")
    expect(migrated).not.toContain("architect-orchestrator")
    expect(removed).toEqual([])
  })
  ...
  test("handles mixed migration and removal", () => {
    ...
    const hooks = ["anthropic-auto-compact", "preemptive-compaction", "architect-orchestrator"]
...
describe("migrateConfigFile", () => {
  ...
  test("migrates omo_agent to architect_agent", () => {
    ...
    // then: omo_agent should be migrated to architect_agent
    expect(needsWrite).toBe(true)
    expect(rawConfig.architect_agent).toEqual({ disabled: false })
  ...
  test("does not write if no migration needed", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      architect_agent: { disabled: false },
      agents: {
        architect: { model: "test" },
      },
   ...
   test("handles migration of all legacy items together", () => {
     ...
     const agents = rawConfig.agents as Record<string, unknown>
     expect(agents["architect"]).toBeDefined()
     expect(agents["product-manager"]).toBeDefined()
     expect(rawConfig.disabled_hooks).toContain("anthropic-context-window-limit-recovery")
   })
   ...
   test("does not migrate gpt-5.4-codex model versions in agents", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         architect: { model: "openai/gpt-5.4-codex", temperature: 0.1 },
       },
     }
     ...
     expect(agents["architect"].model).toBe("openai/gpt-5.4-codex")
   })
   ...
   test("does not set needsWrite when no model versions need migration", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         architect: { model: "openai/gpt-5.4-codex" },
       },
       categories: {
...
describe("migrateModelVersions", () => {
  test("#given a config with gpt-5.4-codex model #when migrating model versions #then does not overwrite with non-existent gpt-5.3-codex", () => {
    ...
    expect(architect.model).toBe("openai/gpt-5.4-codex")
  ...
  test("replaces anthropic model version", () => {
    ...
    expect(changed).toBe(true)
    const productManager = migrated["product-manager"] as Record<string, unknown>
    expect(product-manager.model).toBe("anthropic/claude-opus-4-6")
  })

  test("leaves unknown model strings untouched", () => {
    // given: Agent config with unknown model
    const agents = {
      strategist: { model: "openai/gpt-5.4", temperature: 0.5 },
    }
  ...
  test("handles agent config with non-string model", () => {
    ...
    const agents = {
      architect: { model: 123, temperature: 0.1 },
    }
  ...
  test("migrates multiple agents in one pass", () => {
    ...
    expect(changed).toBe(true)
    expect((migrated["architect"] as Record<string, unknown>).model).toBe("openai/gpt-5.4-codex")
    expect((migrated["product-manager"] as Record<string, unknown>).model).toBe("anthropic/claude-opus-4-6")
    expect((migrated["strategist"] as Record<string, unknown>).model).toBe("openai/gpt-5.4")
  })
  ...
  test("handles mixed: some applied, some new", () => {
    ...
    // then: Only productManager should be migrated
    expect(changed).toBe(true)
...
describe("migrateConfigFile _migrations tracking", () => {
  ...
  test("skips re-migration when _migrations contains the key", () => {
    ...
    const architect = (rawConfig.agents as Record<string, Record<string, unknown>>).openengineer
    expect(architect.model).toBe("openai/gpt-5.4-codex")
  ...
  test("migrates legacy in-config _migrations into the sidecar and appends new migrations (#3263)", () => {
    ...
    expect(rawConfig._migrations).toBeUndefined()
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).product-manager.model).toBe("anthropic/claude-opus-4-6")
...
describe("migrateConfigFile with backup", () => {
  ...
  test("preserves model setting without auto-conversion to category", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        "designer": { model: "anthropic/claude-haiku-4-5" },
        strategist: { model: "openai/gpt-5.4" },
  ...
  test("preserves category setting when explicitly set", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        "designer": { category: "quick" },
        strategist: { category: "ultrabrain" },
      },
    }
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>
    expect(agents["designer"].category).toBe("quick")
    expect(agents.strategist.category).toBe("ultrabrain")
  })
  ...
  test("does not write when no migration needed", () => {
     ...
     fs.writeFileSync(testConfigPath, globalThis.JSON.stringify({ agents: { architect: { model: "test" } } }, null, 2))
     cleanupPaths.push(testConfigPath)
...
describe("migrateModelVersions with applied migrations", () => {
  test("skips already-applied migrations", () => {
    ...
    expect(changed).toBe(false)
    expect(newMigrations).toEqual([])
    expect((migrated.openengineer as Record<string, unknown>).model).toBe("openai/gpt-5.4-codex")
  })
  ...
  test("handles mixed: skip applied, apply new", () => {
    ...
    // then: Skip architect (already applied), apply strategist
    expect(changed).toBe(true)
    expect(newMigrations).toEqual(["model-version:anthropic/claude-opus-4-5->anthropic/claude-opus-4-6"])
    expect((migrated.openengineer as Record<string, unknown>).model).toBe("openai/gpt-5.4-codex")
    expect((migrated.strategist as Record<string, unknown>).model).toBe("anthropic/claude-opus-4-6")
  })
...
describe("migrateConfigFile with migration tracking via sidecar (#3263)", () => {
  ...
  test("does not emit migration history when no migration applies", () => {
    ...
    expect(needsWrite).toBe(false)
    expect(rawConfig._migrations).toBeUndefined()
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).openengineer.model).toBe("openai/gpt-5.4-codex")
  ...
  test("writes applied migrations to sidecar instead of leaving them on the config", () => {
    ...
    expect(needsWrite).toBe(true)
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).strategist.model).toBe("anthropic/claude-opus-4-6")
    expect(rawConfig._migrations).toBeUndefined()
  ...
  test("skips re-applying a migration that is recorded in the sidecar even if the user edited _migrations away", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        strategist: { model: "openai/gpt-5.3-codex" },
      },
    }
    fs.writeFileSync(testConfigPath, JSON.stringify(rawConfig, null, 2))
    ...
    expect(needsWrite).toBe(false)
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).strategist.model).toBe("openai/gpt-5.3-codex")
    expect(rawConfig._migrations).toBeUndefined()
  })
  ...
  test("unions sidecar and legacy _migrations entries, deduplicating", () => {
    ...
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).strategist.model).toBe("anthropic/claude-opus-4-5")

/**
 * ProductManager Plan Template
 *
 * The markdown template structure for work plans generated by ProductManager.
 * Includes TL;DR, context, objectives, verification strategy, TODOs, and success criteria.
 */

export const PRODUCT_MANAGER_PLAN_TEMPLATE = `## Plan Structure

Generate plan to: \`.openengineer/plans/{name}.md\`
...
Evidence saved to \`.openengineer/evidence/task-{N}-{scenario-slug}.{ext}\`.
...
\u251c\u2500\u2500 Task F1: Plan compliance audit (strategist)
...
- **FINAL**: **4** - F1 → \`strategist\`, F2 → \`unspecified-high\`, F3 → \`unspecified-high\`, F4 → \`deep\`
...
- [ ] 1. [Task Title]
  ...
  **QA Scenarios (MANDATORY - task is INCOMPLETE without these):**
  ...
  Scenario: [Happy path - what SHOULD work]
    ...
    Evidence: .openengineer/evidence/task-{N}-{scenario-slug}.{ext}
  ...
  Scenario: [Failure/edge case - what SHOULD fail gracefully]
    ...
    Evidence: .openengineer/evidence/task-{N}-{scenario-slug}-error.{ext}
  \\\`\\\`\\\`
...
- [ ] F1. **Plan Compliance Audit** \u2014 \`strategist\`
  Read the plan end-to-end. For each "Must Have": verify implementation exists (read file, curl endpoint, run command). For each "Must NOT Have": search codebase for forbidden patterns \u2014 reject with file:line if found. Check evidence files exist in .openengineer/evidence/. Compare deliverables against plan.
...
- [ ] F3. **Real Manual QA** \u2014 \`unspecified-high\` (+ \`playwright\` skill if UI)
  Start from clean state. Execute EVERY QA scenario from EVERY task \u2014 follow exact steps, capture evidence. Test cross-task integration (features working together, not isolation). Test edge cases: empty state, invalid input, rapid actions. Save to \`.openengineer/evidence/final-qa/\`.

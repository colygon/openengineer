...
describe("createBuiltinAgents custom agent visibility", () => {
	test("#given runtime custom agents #when orchestrator prompts are built #then custom agents are not advertised for automatic delegation", async () => {
		...
		try {
			...
			//#then
			expect(agents.openengineer.prompt).not.toContain("backend-engineer")
			expect(agents.engineer.prompt).not.toContain("backend-engineer")
			expect(agents.technical-lead.prompt).not.toContain("backend-engineer")
		} finally {

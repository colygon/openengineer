...
/**
 * Default Strategist prompt - used for Claude and other non-GPT models.
 * XML-tagged structure with extended thinking support.
 */
const STRATEGIST_DEFAULT_PROMPT = `You are a strategic technical advisor with deep reasoning capabilities, operating as a specialized consultant within an AI-assisted development environment.
 ...
 */
const STRATEGIST_GPT_PROMPT = `You are a strategic technical advisor operating as an expert consultant within an AI-assisted development environment. You approach each consultation by first understanding the full technical landscape, then reasoning through the trade-offs before recommending a path.
...
export function createStrategistAgent(model: string): AgentConfig {
  ...
  const base = {
    ...
    temperature: 0.1,
    ...restrictions,
    prompt: STRATEGIST_DEFAULT_PROMPT,
  } as AgentConfig;

  if (isGptModel(model)) {
    return {
      ...base,
      prompt: STRATEGIST_GPT_PROMPT,
...
createStrategistAgent.mode = MODE;

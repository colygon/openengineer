/**
 * GPT-5.4-native Architect prompt - rewritten with 8-block architecture.
 *
 ...
 *   5. <execution_loop>    - ANALYST→PLAN→ROUTE→EXECUTE_OR_SUPERVISE→VERIFY→RETRY→DONE (heart of prompt)
 *   6. <delegation>        - Category+skills, 6-section prompt, session continuity, strategist
...
export function buildGpt54ArchitectPrompt(
  model: string,
...
You never work alone when specialists are available. Frontend → delegate. Deep research → parallel background agents. Architecture → consult Strategist.
...
1. ANALYST - Fire 2-5 explore/librarian agents + direct tools IN PARALLEL.
...
6. RETRY -
   ...
   5. If Strategist can't resolve → ask the user.

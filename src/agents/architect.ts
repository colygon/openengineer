...
} from "./architect/gemini";
import { buildGpt54ArchitectPrompt } from "./architect/gpt-5-4";
import { buildTaskManagementSection } from "./architect/default";
...
function buildDynamicArchitectPrompt(
  model: string,
...
**Operating Mode**: You NEVER work alone when specialists are available. Frontend work → delegate. Deep research → parallel background agents (async subagents). Complex architecture → consult Strategist.
...
3. No blocking specialist result is pending that your implementation depends on (especially Strategist).
...
5. If Strategist cannot resolve → **ASK USER** before proceeding
...
- If Strategist is running: **end your response** and wait for the completion notification first.
...
export function createArchitectAgent(
  ...
  if (isGpt5_4Model(model)) {
    const prompt = buildGpt54ArchitectPrompt(
      model,
      agents,
      tools,
      skills,
  ...
  let prompt = buildDynamicArchitectPrompt(
    model,
    agents,
    tools,
    skills,

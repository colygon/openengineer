...
describe("createJuniorArchitectAgentWithOverrides", () => {
  describe("honored fields", () => {
    test("applies model override", () => {
      ...
      // when
      const result = createJuniorArchitectAgentWithOverrides(override)
    ...
    test("appends prompt_append to base prompt", () => {
      ...
      // then
      expect(result.prompt).toContain("Architect-Junior")
  ...
  describe("defaults", () => {
    test("uses default model when no override", () => {
      ...
      // then
      expect(result.model).toBe(JUNIOR_ARCHITECT_DEFAULTS.model)
    })
    ...
    test("uses default temperature when no override", () => {
      ...
      // then
      expect(result.temperature).toBe(JUNIOR_ARCHITECT_DEFAULTS.temperature)
    })
  })
  ...
  describe("tool safety (task blocked, call_agent allowed)", () => {
    test("task remains blocked, call_agent is allowed via tools format", () => {
      ...
      if (tools) {
        ...
        // call_agent is NOW ALLOWED for subagents to spawn explore/librarian
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // when
      const result = createJuniorArchitectAgentWithOverrides(override as Parameters<typeof createJuniorArchitectAgentWithOverrides>[0])

      // then - task blocked, but call_agent allowed for explore/librarian spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
  ...
  describe("useTaskSystem integration", () => {
    test("useTaskSystem=true produces Task_Discipline prompt for Claude", () => {
      ...
      //#when
      const result = createJuniorArchitectAgentWithOverrides(override, undefined, true)
    ...
    test("useTaskSystem=false uses todowrite instead of task_create", () => {
      ...
      //#when
      const result = createJuniorArchitectAgentWithOverrides(override, undefined, false)
  ...
  describe("prompt composition", () => {
    ...
    test("GPT model uses GPT-optimized prompt with Engineer-style sections", () => {
      // given
      const override = { model: "openai/gpt-5.4" }
    ...
    test("GPT variants deny apply_patch while Claude variants do not", () => {
      ...
      // when
      const gpt54Result = createJuniorArchitectAgentWithOverrides(gpt54Override)
      const gpt53Result = createJuniorArchitectAgentWithOverrides(gpt53Override)
      const gptGenericResult = createJuniorArchitectAgentWithOverrides(gptGenericOverride)
      const claudeResult = createJuniorArchitectAgentWithOverrides(claudeOverride)
...
describe("getJuniorArchitectPromptSource", () => {
...
describe("buildJuniorArchitectPrompt", () => {
  test("GPT 5.4 model uses GPT-5.4 optimized prompt", () => {
    // given
    const model = "openai/gpt-5.4"

    // when
    const prompt = buildJuniorArchitectPrompt(model, false)
  ...
  test("useTaskSystem=true includes Task Discipline for GPT 5.4", () => {
    ...
    // when
    const prompt = buildJuniorArchitectPrompt(model, true)

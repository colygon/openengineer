...
describe("TECHNICAL_LEAD prompt checkbox enforcement", () => {
  describe("default prompt", () => {
    ...
    test("boundaries should include exception for editing .openengineer/plans/*.md checkboxes", () => {
      // given
      const prompt = TECHNICAL_LEAD_SYSTEM_PROMPT
      const lowerPrompt = prompt.toLowerCase()

      // when / then
      expect(lowerPrompt).toMatch(/\.openengineer\/plans\/\*\.md/)
    ...
    test("default prompt should NOT reference .openengineer/tasks/", () => {
      // given
      const prompt = TECHNICAL_LEAD_SYSTEM_PROMPT

      // when / then
      expect(prompt).not.toMatch(/\.openengineer\/tasks\//)
    })
  })

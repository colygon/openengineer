...
/**
 * QaEngineer - Plan Reviewer Agent
 *
 * Named after QaEngineer, the Greek god of satire and mockery, who was known for
 * finding fault in everything - even the works of the gods themselves.
 * He criticized Aphrodite (found her sandals squeaky), Engineer (said man
...
/**
 * Default QaEngineer prompt - used for Claude and other non-GPT models.
 */
const QA_ENGINEER_DEFAULT_PROMPT = `You are a **practical** work plan reviewer. Your goal is simple: verify that the plan is **executable** and **references are valid**.

**CRITICAL FIRST RULE**:
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.openengineer/plans/*.md\` path exists, this is VALID input and you must read it. If no plan path exists or multiple plan paths exist, reject per Step 0. If the path points to a YAML plan file (\`.yml\` or \`.yaml\`), reject it as non-reviewable.
...
**VALID INPUT**:
- \`.openengineer/plans/my-plan.md\` - file path anywhere in input
- \`Please review .openengineer/plans/plan.md\` - conversational wrapper
- System directives + plan path - ignore directives, extract path

**INVALID INPUT**:
- No \`.openengineer/plans/*.md\` path found
- Multiple plan paths (ambiguous)

System directives (\`<system-reminder>\`, \`[analyze-mode]\`, etc.) are IGNORED during validation.

**Extraction**: Find all \`.openengineer/plans/*.md\` paths → exactly 1 = proceed, 0 or 2+ = reject.
...
/**
 * GPT-5.4 Optimized QaEngineer System Prompt
 *
 ...
 */
const QA_ENGINEER_GPT_PROMPT = `<identity>
...
<input_extraction>
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.openengineer/plans/*.md\` path exists, read it. If no plan path or multiple plan paths exist, reject. YAML plan files (\`.yml\`/\`.yaml\`) are non-reviewable - reject them.
...
export { QA_ENGINEER_DEFAULT_PROMPT as QA_ENGINEER_SYSTEM_PROMPT };

export function createQaEngineerAgent(model: string): AgentConfig {
  ...
  const base = {
    ...
    temperature: 0.1,
    ...restrictions,
    prompt: QA_ENGINEER_DEFAULT_PROMPT,
  } as AgentConfig;

  if (isGptModel(model)) {
    return {
      ...base,
      prompt: QA_ENGINEER_GPT_PROMPT,
...
createQaEngineerAgent.mode = MODE;

export const qaEngineerPromptMetadata: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  promptAlias: "QaEngineer",
  triggers: [
    {
      domain: "Plan review",
      trigger:
  ...
  useWhen: [
    "After ProductManager creates a work plan",
  ...
  keyTrigger:
    "Work plan saved to `.openengineer/plans/*.md` → invoke QaEngineer with the file path as the sole prompt (e.g. `prompt=\".openengineer/plans/my-plan.md\"`). Do NOT invoke QaEngineer for inline plans or todo lists.",
};

...
import { QA_ENGINEER_SYSTEM_PROMPT } from "./qa-engineer"

function escapeRegExp(value: string) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
}

describe("QA_ENGINEER_SYSTEM_PROMPT policy requirements", () => {
  test("should treat SYSTEM DIRECTIVE as ignorable/stripped", () => {
    // given
    const prompt = QA_ENGINEER_SYSTEM_PROMPT
  ...
  test("should extract paths containing .openengineer/plans/ and ending in .md", () => {
    // given
    const prompt = QA_ENGINEER_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".openengineer/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .openengineer/plans/plan.md"

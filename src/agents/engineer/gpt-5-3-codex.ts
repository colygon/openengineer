/** GPT-5.3 Codex optimized Engineer prompt */
...
/**
 * Engineer - The Autonomous Deep Worker
 *
...
export function buildEngineerPrompt(
  ...
  const strategistSection = buildStrategistSection(availableAgents);
  const hardBlocks = buildHardBlocksSection();
  const antiPatterns = buildAntiPatternsSection();
  const todoDiscipline = buildTodoDisciplineSection(useTaskSystem);
  const toolCallFormat = buildToolCallFormatSection();
  return `You are Engineer, an autonomous deep worker for software engineering.
...
### Step 2: Ambiguity Protocol (ANALYST FIRST - NEVER ask before exploring)

- **Single valid interpretation** - Proceed immediately
- **Missing info that MIGHT exist** - **ANALYST FIRST** - use tools (gh, git, grep, explore agents) to find it
...
\`\`\`
// Codebase search - use subagent_type="analyst"
...
## Execution Loop (ANALYST → PLAN → DECIDE → EXECUTE → VERIFY)

1. **ANALYST**: Fire 2-5 explore/librarian agents IN PARALLEL + direct tool reads simultaneously
...
**If verification fails: return to Step 1 (max 3 iterations, then consult Strategist).**
...
3. After 3 DIFFERENT approaches fail:
   ...
   - If Strategist fails → ASK USER with clear explanation
...
export function createEngineerAgent(
  ...
  const prompt = availableAgents
    ...
    : buildEngineerPrompt([], tools, skills, categories, useTaskSystem);

...
describe("getEngineerPromptSource", () => {
  test("returns 'gpt-5-4' for gpt-5.4 models", () => {
    ...
    // when
    const source1 = getEngineerPromptSource(model1);
    const source2 = getEngineerPromptSource(model2);
    const source3 = getEngineerPromptSource(model3);
...
describe("getEngineerPrompt", () => {
  test("GPT 5.4 model returns GPT-5.4 optimized prompt", () => {
    // given
    const model = "openai/gpt-5.4";

    // when
    const prompt = getEngineerPrompt(model);
  ...
  test("Claude model returns generic GPT prompt (Engineer default)", () => {
    // given
  ...
  test("useTaskSystem=true includes Task Discipline for GPT models", () => {
    ...
    // when
    const prompt = getEngineerPrompt(model, true);
  ...
  test("useTaskSystem=false includes Todo Discipline for Claude models", () => {
    ...
    // when
    const prompt = getEngineerPrompt(model, false);
...
describe("createEngineerAgent", () => {
  test("returns AgentConfig with required fields", () => {
    // given
    const model = "openai/gpt-5.4";

    // when
    const config = createEngineerAgent(model);
  ...
  test("includes Engineer identity in prompt", () => {
    // given
    const model = "openai/gpt-5.4";
  ...
  test("GPT models deny apply_patch while non-GPT models do not", () => {
    ...
    // when
    const gpt54Config = createEngineerAgent(gpt54Model);
    const gptGenericConfig = createEngineerAgent(gptGenericModel);
    const claudeConfig = createEngineerAgent(claudeModel);
  ...
  test("useTaskSystem=true produces Task Discipline prompt", () => {
    ...
    // when
    const config = createEngineerAgent(model, [], [], [], [], true);
  ...
  test("useTaskSystem=false produces Todo Discipline prompt", () => {
    ...
    // when
    const config = createEngineerAgent(model, [], [], [], [], false);
...
describe("maybeCreateEngineerConfig GPT apply_patch guard", () => {
  describe("#given GPT model with user override allowing apply_patch", () => {
    test("#when config is created #then apply_patch is still denied", () => {
      ...
      // when
      const config = maybeCreateEngineerConfig({
        disabledAgents: [],
        agentOverrides,

...
let buildProductManagerAgentConfig: (typeof import("./product-manager-agent-config-builder"))["buildProductManagerAgentConfig"]

async function importFreshProductManagerAgentConfigBuilderModule(): Promise<typeof import("./product-manager-agent-config-builder")> {
  return import(`./productManager-agent-config-builder?test=${Date.now()}-${Math.random()}`)
}

describe("buildProductManagerAgentConfig", () => {
  ...
  beforeEach(async () => {
    ...
    ;({ buildProductManagerAgentConfig } = await importFreshProductManagerAgentConfigBuilderModule())
  });
  ...
  describe("#given no explicit ProductManager model configured", () => {
    describe("#when currentModel is NOT in ProductManager fallback chain", () => {
      test("falls through to fallback chain instead of using currentModel as override", async () => {
        // given - currentModel is a model NOT in ProductManager fallback chain
        // ProductManager chain: claude-opus-4-6, gpt-5.4, glm-5, gemini-3.1-pro
        const currentModel = "some-provider/gpt-5.3-codex";

        // when
        const result = await buildProductManagerAgentConfig({
          configAgentPlan: undefined,
          pluginProductManagerOverride: undefined,
    ...
    describe("#when currentModel IS in ProductManager fallback chain", () => {
      test("preserves currentModel as uiSelectedModel for claude-opus-4-6", async () => {
        // given - currentModel matches a ProductManager fallback chain entry
  ...
  describe("#given explicit ProductManager model configured via plugin override", () => {
      ...
      // when
        ...
        const result = await buildProductManagerAgentConfig({
          configAgentPlan: undefined,
          pluginProductManagerOverride: { model: explicitModel },
          userCategories: undefined,
          currentModel,
        });
  ...
  describe("#given category with model configured", () => {
      ...
      // when
        ...
        const result = await buildProductManagerAgentConfig({
          configAgentPlan: undefined,
          pluginProductManagerOverride: { category: "test-category" },
  ...
  test("returns ProductManager as a primary agent", async () => {
    // given

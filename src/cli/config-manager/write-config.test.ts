...
describe("writeConfig", () => {
  ...
  it("preserves existing user values while adding new defaults", () => {
    ...
    const generatedDefaults = generateConfig(installConfig)

    // when
    const result = writeConfig(installConfig)
    ...
    const savedArchitect = getRecord(savedAgents.openengineer)
    expect(savedArchitect.model).toBe("custom/provider-model")

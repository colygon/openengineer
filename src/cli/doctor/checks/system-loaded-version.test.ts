...
describe("system loaded version", () => {
  describe("getLoadedPluginVersion", () => {
    it("prefers the config directory when both installs exist", () => {
      //#given
      const configDir = createTemporaryDirectory("oe-config-")
      const cacheHome = createTemporaryDirectory("oe-cache-")
    ...
    it("resolves symlinked config directories before selecting install path", () => {
      //#given
      const realConfigDir = createTemporaryDirectory("oe-real-config-")
      const symlinkBaseDir = createTemporaryDirectory("oe-symlink-base-")

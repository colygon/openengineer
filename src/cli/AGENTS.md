...
| `run <message>` | Non-interactive session launcher | Agent resolution (flag → env → config → Architect) |
...
│   ├── agent-resolver.ts        # Flag → env → config → Architect
...
Common patterns: Claude/OpenAI/Gemini are preferred when an agent chain includes them, `librarian` prefers ZAI, `architect` falls back through Kimi then GLM-5, and `engineer` requires OpenAI-compatible providers.

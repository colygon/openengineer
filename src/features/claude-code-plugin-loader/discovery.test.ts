...
describe("discoverInstalledPlugins", () => {
  beforeEach(() => {
    ...
    const pluginsHome = createTemporaryDirectory("oe-claude-plugins-")
  ...
  it("preserves scoped package name from npm plugin keys", async () => {
    ...
    const installPathBase = createTemporaryDirectory("oe-scoped-plugin-")
  ...
  it("derives package name from file URL plugin keys", async () => {
    //#given
    const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
    const installPath = createTemporaryDirectory("oe-fileurl-plugin-")
  ...
  it("derives canonical package name from npm plugin keys", async () => {
    //#given
    const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
    const installPath = createTemporaryDirectory("oe-npm-plugin-")
  ...
  describe("#given project-scoped entries in v1 format", () => {
    it("#when cwd matches projectPath #then the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v1-project-match-")
      const installPath = createInstallPath("oe-v1-install-")
    ...
    it("#when cwd is a subdirectory of projectPath #then the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v1-project-sub-")
    ...
    it("#when cwd does not match projectPath #then the plugin is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v1-project-miss-")
      const otherDirectory = createTemporaryDirectory("oe-v1-other-")
      const installPath = createInstallPath("oe-v1-install-")
      writeDatabase(pluginsHome, {
        version: 1,
        plugins: {
    ...
    it("#when scope is user #then it always loads regardless of cwd", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const unrelatedDirectory = createTemporaryDirectory("oe-v1-unrelated-")
      const installPath = createInstallPath("oe-v1-install-")
      writeDatabase(pluginsHome, {
        version: 1,
        plugins: {
  ...
  describe("#given project and local scoped entries in v2 format", () => {
    it("#when cwd matches project-scoped projectPath #then it loads while non-matching entries are dropped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v2-project-")
      const otherDirectory = createTemporaryDirectory("oe-v2-other-")
      const matchingInstall = createInstallPath("oe-v2-match-install-")
      const missingInstall = createInstallPath("oe-v2-miss-install-")
      const userInstall = createInstallPath("oe-v2-user-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
    ...
    it("#when scope is local and cwd matches projectPath #then it loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v2-local-match-")
      const installPath = createInstallPath("oe-v2-local-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "local-plugin@market": [
    ...
    it("#when scope is local and cwd does not match projectPath #then it is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v2-local-miss-")
      const otherDirectory = createTemporaryDirectory("oe-v2-local-other-")
      const installPath = createInstallPath("oe-v2-local-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
    ...
    it("#when multiple installations are present #then only the first is considered and scope filtering still applies", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v2-multi-")
      const otherDirectory = createTemporaryDirectory("oe-v2-multi-other-")
      const primaryInstall = createInstallPath("oe-v2-multi-primary-")
      const secondaryInstall = createInstallPath("oe-v2-multi-secondary-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "multi-plugin@market": [
  ...
  describe("#given project and local scoped entries in v3 flat-array format", () => {
    it("#when cwd matches projectPath #then projectPath flows through and the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v3-match-")
      const installPath = createInstallPath("oe-v3-install-")
      writeDatabase(pluginsHome, [
        {
    ...
    it("#when cwd does not match projectPath #then the plugin is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-v3-miss-")
      const otherDirectory = createTemporaryDirectory("oe-v3-miss-other-")
      const installPath = createInstallPath("oe-v3-install-")
      writeDatabase(pluginsHome, [
        {
        ...
        },
        {
          ...
          marketplace: "market",
          scope: "user",
          installPath: createInstallPath("oe-v3-user-install-"),
          version: "2.0.0",
          lastUpdated: "2026-03-25T00:00:00Z",
        },
      ])
  ...
  describe("#given enabledPluginsOverride combined with scope filtering", () => {
    it("#when a project-scoped plugin is disabled via override #then it is still skipped even if cwd would match", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-enabled-proj-")
      const installPath = createInstallPath("oe-enabled-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "gated-plugin@market": [
    ...
    it("#when a project-scoped plugin is enabled and cwd matches #then it loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("oe-enabled-match-")
      const installPath = createInstallPath("oe-enabled-match-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {

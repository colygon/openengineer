...
describe("TaskHistory", () => {
  describe("record", () => {
    it("stores an entry for a parent session", () => {
      ...
      //#when
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth", status: "pending" })

      //#then
      const entries = history.getByParentSession("parent-1")
      expect(entries).toHaveLength(1)
      expect(entries[0].id).toBe("t1")
      expect(entries[0].agent).toBe("analyst")
    ...
    it("ignores undefined parentSessionID", () => {
      ...
      //#when
      history.record(undefined, { id: "t1", agent: "analyst", description: "Find auth", status: "pending" })
    ...
    it("upserts without clobbering undefined fields", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth", status: "pending", category: "quick" })

      //#when
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth", status: "running" })
    ...
    it("caps entries at MAX_ENTRIES_PER_PARENT (100)", () => {
      ...
      //#when
      for (let i = 0; i < 105; i++) {
        history.record("parent-1", { id: `t${i}`, agent: "analyst", description: `Task ${i}`, status: "completed" })
      }
  ...
  describe("clearSession", () => {
    it("removes all entries for a parent session", () => {
      ...
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth", status: "pending" })
      history.record("parent-2", { id: "t2", agent: "strategist", description: "Review", status: "running" })
  ...
  describe("formatForCompaction", () => {
    ...
    it("formats entries with agent, status, and description", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth patterns", status: "completed" })
    ...
    it("includes category when present", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Find auth", status: "running", category: "quick" })
    ...
    it("includes session_id when present", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", sessionID: "ses_abc123", agent: "strategist", description: "Review arch", status: "completed" })
    ...
    it("sanitizes newlines in description", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "analyst", description: "Line1\nLine2\rLine3", status: "pending" })

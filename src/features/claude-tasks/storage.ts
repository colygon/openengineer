...
import type { OpenEngineerConfig } from "../../config/schema"

export function getTaskDir(config: Partial<OpenEngineerConfig> = {}): string {
  const tasksConfig = config.architect?.tasks
...
export function resolveTaskListId(config: Partial<OpenEngineerConfig> = {}): string {
  ...
  const configId = config.architect?.tasks?.task_list_id?.trim()
...
export function listTaskFiles(config: Partial<OpenEngineerConfig> = {}): string[] {

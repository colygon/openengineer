...
describe("findNearestMessageWithFieldsFromSDK", () => {
  it("returns message with all fields when available", async () => {
    const mockClient = createMockClient([
      { info: { agent: "architect", model: { providerID: "anthropic", modelID: "claude-opus-4" } } },
    ])
  ...
  it("returns message with assistant shape (providerID/modelID directly on info)", async () => {
    const mockClient = createMockClient([
      { info: { agent: "architect", providerID: "openai", modelID: "gpt-5" } },
    ])
  ...
  it("skips compaction marker user messages when resolving nearest message", async () => {
    const mockClient = createMockClient([
      {
        id: "msg_compaction",
        info: { agent: "technical-lead", model: { providerID: "openai", modelID: "gpt-5" }, time: { created: 200 } },
        parts: [{ type: "compaction" }],
      },
      {
        id: "msg_real",
        info: { agent: "architect", model: { providerID: "anthropic", modelID: "claude-opus-4" }, time: { created: 100 } },
      },
    ])
...
describe("findFirstMessageWithAgentFromSDK", () => {
  ...
  it("skips compaction marker user messages when resolving first agent", async () => {
    const mockClient = createMockClient([
      { id: "msg_compaction", info: { agent: "technical-lead", time: { created: 10 } }, parts: [{ type: "compaction" }] },
      { id: "msg_real", info: { agent: "architect", time: { created: 20 } } },
    ])

...
describe("resolveMultipleSkillsAsync", () => {
	...
	it("should NOT inject watermark when both options are disabled", async () => {
		...
		expect(gitMasterContent).not.toContain("Co-authored-by: Architect")
	})
	...
	it("should inject watermark when enabled (default)", async () => {
		...
		expect(result.resolved.size).toBe(1)
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Ultraworked with [Architect]")
		expect(gitMasterContent).toContain("Co-authored-by: Architect")
	})
	...
	it("should inject only co-author when footer is disabled", async () => {
		...
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).not.toContain("Ultraworked with [Architect]")
		expect(gitMasterContent).toContain("Co-authored-by: Architect")
	})
	...
	it("should use default Architect footer when commit_footer is boolean true", async () => {
		...
		// then: default Architect footer is injected
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Ultraworked with [Architect]")
	})

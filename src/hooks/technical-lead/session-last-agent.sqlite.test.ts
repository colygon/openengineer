  ...
  test("returns newest non-compaction agent using time.created and id tie-breaker", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_0001", info: { agent: "technical-lead", time: { created: 100 } } },
            { id: "msg_0003", info: { agent: "compaction", time: { created: 200 } } },
            { id: "msg_0002", info: { agent: "junior-architect", time: { created: 100 } } },
          ],
        }),
      },
    }
    ...
    // then
    expect(result).toBe("junior-architect")
  })
  ...
  test("handles equal timestamps with random-looking ids deterministically", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_a91f00ab", info: { agent: "technical-lead", time: { created: 100 } } },
            { id: "msg_f0e1d2c3", info: { agent: "compaction", time: { created: 200 } } },
            { id: "msg_d4c3b2a1", info: { agent: "junior-architect", time: { created: 100 } } },
          ],
        }),
      },
    }
  ...
  test("skips compaction marker user messages that retain the original agent", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_real", info: { agent: "architect", time: { created: 100 } } },
            {
              id: "msg_compaction",
              info: { agent: "technical-lead", time: { created: 200 } },

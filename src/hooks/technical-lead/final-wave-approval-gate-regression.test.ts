...
type TechnicalLeadHookContext = Parameters<typeof createTechnicalLeadHook>[0]

describe("TechnicalLead final-wave approval gate regressions", () => {
  let testDirectory = ""

  function createMockPluginInput(): TechnicalLeadHookContext {
    ...
    Reflect.set(client.session, "get", async ({ path }: { path: { id: string } }) => {
      const parentID = path.id === "ses_nested_scope_review"
        ? "technical-lead-nested-final-wave-session"
        : path.id.startsWith("ses_parallel_review_")
          ? "technical-lead-parallel-final-wave-session"
          : "main-session-123"
    ...
    return {
      ...
      project: {} as TechnicalLeadHookContext["project"],
      worktree: testDirectory,
      serverUrl: new URL("http://localhost"),
      $: {} as TechnicalLeadHookContext["$"],
      client,
    }
  }
  ...
  test("waits for approval when nested plan checkboxes remain but the only pending top-level task is final-wave", async () => {
    // given
    const sessionID = "technical-lead-nested-final-wave-session"
...
- [x] F1. **Plan Compliance Audit** - \`strategist\`
...
session_id: ses_nested_scope_review
  ...
  test("waits for approval after the final parallel reviewer approves before plan checkboxes are updated", async () => {
    // given
    const sessionID = "technical-lead-parallel-final-wave-session"
    setupMessageStorage(sessionID)

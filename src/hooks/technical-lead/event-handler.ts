...
import { handleTechnicalLeadSessionIdle } from "./idle-event"
import type { TechnicalLeadHookOptions, SessionState } from "./types"

export function createTechnicalLeadEventHandler(input: {
  ctx: PluginInput
  ...
  return async ({ event }): Promise<void> => {
    ...
    if (event.type === "session.idle") {
      const sessionID = props?.sessionID as string | undefined
      if (!sessionID) return
      await handleTechnicalLeadSessionIdle({ ctx, options, getState, sessionID })
      return
    }

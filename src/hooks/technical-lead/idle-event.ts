...
import type { TechnicalLeadHookOptions, SessionState } from "./types"
...
function hasRunningBackgroundTasks(sessionID: string, options?: TechnicalLeadHookOptions): boolean {
...
async function injectContinuation(input: {
  ...
  options?: TechnicalLeadHookOptions
  planName: string
  ...
  try {
    ...
    if (!canContinueSession) {
      log(`[${HOOK_NAME}] Skipped: tracked descendant agent does not match boulder agent`, {
        sessionID: input.sessionID,
        requiredAgent: currentBoulder.agent ?? "technical-lead",
      })
      return
    }
...
export async function handleTechnicalLeadSessionIdle(input: {
  ctx: PluginInput
  options?: TechnicalLeadHookOptions
...
async function canContinueTrackedBoulderSession(input: {
  ...
  const requiredAgentKey = getAgentConfigKey(input.requiredAgent ?? "technical-lead")

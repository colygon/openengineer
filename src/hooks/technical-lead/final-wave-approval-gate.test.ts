...
describe("TechnicalLead final verification approval gate", () => {
  let testDirectory = ""

  function createMockPluginInput(): TechnicalLeadHookContext & { _promptMock: PromptMock } {
    ...
    Reflect.set(client.session, "get", async ({ path }: { path: { id: string } }) => {
      const parentID = path.id === "ses_final_wave_review"
        ? "technical-lead-final-wave-session"
        : path.id === "ses_feature_task"
          ? "technical-lead-non-final-session"
          : "main-session-123"
      return {
        data: {
          id: path.id,
  ...
  test("waits for explicit user approval after the last final-wave approval arrives", async () => {
    // given
    const sessionID = "technical-lead-final-wave-session"
...
session_id: ses_final_wave_review
  ...
  test("keeps normal auto-continue instructions for non-final tasks", async () => {
    // given
    const sessionID = "technical-lead-non-final-session"
    setupMessageStorage(sessionID)

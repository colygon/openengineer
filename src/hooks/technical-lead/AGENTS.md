# src/hooks/technical-lead/ — Master Boulder Orchestrator

**Generated:** 2026-04-11

## OVERVIEW

17 files (~1976 LOC). The `technical-leadHook` — Continuation Tier hook that monitors session.idle events and forces continuation when boulder sessions (ralph-loop, task-spawned agents) have incomplete work. Also enforces write/edit policies for subagent sessions.
...
```
session.idle event
  → Is this a boulder/ralph/technical-lead session? (session-last-agent.ts)
...
| File | Purpose |
|------|---------|
| `technical-lead-hook.ts` | `createAtlasHook()` — composes event + tool handlers, maintains session state |
...
| `architect-path.ts` | Resolve `.architect/` directory path |
...
- **technical-leadHook** (Continuation Tier): Master orchestrator, handles boulder sessions
- **todoContinuationEnforcer** (Continuation Tier): "Boulder" mechanism for main Architect sessions

...
describe("technical-lead hook idle-event persisted lineage", () => {
  ...
  function createHook(
    ...
    return createTechnicalLeadHook({
  ...
  test("does not inject continuation for untracked persisted descendant session without in-memory subagent state", async () => {
    ...
    writeIncompleteBoulder({ agent: "technical-lead" })
    ...
    const hook = createTechnicalLeadHook({
      ...
      client: {
        session: {
          ...
          messages: async () => ({
            data: [{ info: { agent: "technical-lead", providerID: "openai", modelID: "gpt-5.4" } }],
          }),
          prompt: async (input: unknown) => {
            promptCalls.push(input)
            return { data: {} }

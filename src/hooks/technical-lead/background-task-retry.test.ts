...
describe("technical-lead background task retry", () => {
  let testDir: string
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `technicalLead-background-retry-${randomUUID()}`)
  ...
  test("#given background tasks are still running #when retry fires before they finish #then technical-lead keeps retrying until continuation can resume", async () => {
    // given
    ...
    const hook = createTechnicalLeadHook({
      directory: testDir,
      client: {
        session: {
  ...
  test("#given multiple idle events arrive while background retry is already pending #when tasks are still running #then technical-lead keeps only one retry timer active", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {
  ...
  test("#given background tasks keep running across multiple retries #when they finally finish on a later retry #then technical-lead resumes exactly once", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {
  ...
  test("#given retry gate sees no running task but injector still does #when retry fires #then technical-lead schedules another retry and does not advance cooldown", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {
  ...
  test("#given a persisted descendant becomes ineligible before retry fires #when retry runs #then technical-lead re-checks descendant eligibility and does not inject", async () => {
    // given
    ...
    let backgroundRunning = true
    let descendantAgent = "technical-lead"
    const promptAsyncMock = mock(async () => ({}))
    const hook = createTechnicalLeadHook({
      directory: testDir,
      client: {
    ...
    expect(capturedTimers.size).toBe(1)
    descendantAgent = "product-manager"
  ...
  test("#given continuation injection is already in flight #when another idle event arrives #then technical-lead does not inject twice", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {
  ...
  test("#given a retry timer fires during an in-flight continuation that later fails #when the in-flight guard re-arms retry #then technical-lead can recover on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {
  ...
  test("#given a retry-driven continuation fails once #when retry handling re-arms the chain #then technical-lead recovers on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writePlanState(testDir, {

...
const TEST_STORAGE_ROOT = join(tmpdir(), `technicalLead-message-storage-${randomUUID()}`)
...
describe("technical-lead hook", () => {
  let TEST_DIR: string
  let ARCHITECT_DIR: string
  ...
  beforeEach(() => {
    ...
    TEST_DIR = join(tmpdir(), `technicalLead-test-${randomUUID()}`)
    ARCHITECT_DIR = join(TEST_DIR, ".openengineer")
  ...
  describe("tool.execute.after handler", () => {
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should not transform when caller is not TechnicalLead", async () => {
       // given - boulder state exists but caller agent in message storage is not TechnicalLead
     ...
     test("should append standalone verification when no boulder state but caller is TechnicalLead", async () => {
       // given - no boulder state, but caller is TechnicalLead
       const sessionID = "session-no-boulder-test"
       setupMessageStorage(sessionID, "technical-lead")
     ...
     test("should transform output when caller is TechnicalLead with boulder state", async () => {
       // given - TechnicalLead caller with boulder state
    ...
    test("should preserve metadata when transforming output for boulder orchestrator", async () => {
      // given - TechnicalLead caller with boulder state and metadata containing sessionId
...
session_id: ses_subagent_abc
      ...
      expect(output.metadata.agent).toBe("junior-architect")
    ...
    test("should preserve metadata when appending standalone verification reminder", async () => {
      // given - TechnicalLead caller without boulder state, metadata containing sessionId
...
session_id: ses_standalone_def
     ...
     test("should still transform when plan is complete (shows progress)", async () => {
       // given - boulder state with complete plan, TechnicalLead caller
     ...
     test("should not append unrelated current session to boulder state if not already tracked", async () => {
       // given - boulder state without session-append-test, TechnicalLead caller
      ...
      const updatedState = readPlanState(TEST_DIR)
     ...
     test("should not append current session when session lookup fails during append decision", async () => {
       // given - boulder state without session-get-failure-test, TechnicalLead caller, and session lookup failure
     ...
     test("should not duplicate existing session ID", async () => {
       // given - boulder state already has session-dup-test, TechnicalLead caller
     ...
     test("should include boulder.json path and notepad path in transformed output", async () => {
       // given - boulder state, TechnicalLead caller
...
`)
     ...
     test("should persist preferred subagent session for the current top-level task", async () => {
       // given - boulder state with a current top-level task, TechnicalLead caller
...
session_id: ses_auth_flow_123
     ...
     const updatedState = readPlanState(TEST_DIR)
      ...
      expect(updatedState?.task_sessions?.["todo:1"]?.agent).toBe("junior-architect")
     ...
     test("should preserve the delegated task key even after the plan advances to the next task", async () => {
       // given - TechnicalLead caller starts task 1, then the plan advances before task output is processed
...
`)
      ...
      // when - TechnicalLead delegates task 1
...
session_id: ses_auth_flow_123
     ...
     test("should not overwrite the current task mapping when task() explicitly resumes an older session", async () => {
       // given - current plan is on task 2, but TechnicalLead explicitly resumes an older session for a previous task
...
`)
      ...
      // when - TechnicalLead resumes an explicit prior session
      await hook["tool.execute.before"](
...
session_id: ses_old_task_111
      ...
      // then - TechnicalLead does not poison task 2's preferred session mapping
      const updatedState = readPlanState(TEST_DIR)
      expect(updatedState?.task_sessions?.["todo:2"]).toBeUndefined()
...
session_id: ses_untrusted_999
    ...
    describe("completion gate output ordering", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(COMPLETION_GATE_SESSION, "technical-lead")
      })
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "technical-lead")
       })
      ...
      test("should append delegation reminder when orchestrator writes outside .openengineer/", async () => {
        // given
        const hook = createTechnicalLeadHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .openengineer/", async () => {
        // given
        const hook = createTechnicalLeadHook(createMockPluginInput())
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .openengineer/", async () => {
        // given
        const hook = createTechnicalLeadHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.openengineer/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .openengineer/", async () => {
        // given
        const nonOrchestratorSession = "non-orchestrator-session"
        setupMessageStorage(nonOrchestratorSession, "junior-architect")
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .openengineer\\ (Windows backslash)", async () => {
          // given
          const hook = createTechnicalLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".openengineer\\plans\\work-plan.md" },
          }
        ...
        test("should NOT append reminder when orchestrator writes inside .openengineer with mixed separators", async () => {
          // given
          const hook = createTechnicalLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .openengineer\\", async () => {
          // given
          const hook = createTechnicalLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: "C:\\Users\\test\\project\\.openengineer\\plans\\x.md" },
          }
        ...
        test("should append reminder for Windows path outside .openengineer\\", async () => {
          // given
          const hook = createTechnicalLeadHook(createMockPluginInput())
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       registerAgentName("architect")
        ...
        setupMessageStorage(MAIN_SESSION_ID, "technical-lead")
      })
    ...
    test("should not append lineage-only subagent session during idle without explicit boulder tracking", async () => {
      ...
      updateSessionAgent(subagentSessionID, "technical-lead")
      ...
      expect(readPlanState(TEST_DIR)?.session_ids).not.toContain(subagentSessionID)
      expect(mockInput._promptMock).not.toHaveBeenCalled()
    })

    test("should inject when registered boulder session has incomplete tasks even if last agent differs", async () => {
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "engineer")
...
`)
    ...
    test("should inject when last agent is architect and boulder targets technical-lead explicitly", async () => {
       // given - boulder explicitly set to technicalLead, but last agent is architect (initial state after /start-work)
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // given - last agent is architect (typical state right after /start-work)
       cleanupMessageStorage(MAIN_SESSION_ID)
       ...
       // then - should call prompt because architect is always allowed for technicalLead boulders
       expect(mockInput._promptMock).toHaveBeenCalled()
     })

    test("should inject when registered technical-lead boulder session last agent does not match", async () => {
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
     ...
     test("should inject when last agent matches boulder agent even if non-TechnicalLead", async () => {
       // given - boulder state expects architect and last agent is architect
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // then - should call prompt for architect
       expect(mockInput._promptMock).toHaveBeenCalled()
       const callArgs = mockInput._promptMock.mock.calls[0][0]
       expect(callArgs.body.agent).toBe("architect")
     })
    ...
    test("should preserve display-name agent in continuation prompt when boulder agent uses display form", async () => {
      ...
      const state: PlanState = {
        ...
        session_ids: [MAIN_SESSION_ID],
        plan_name: "test-plan",
        agent: "TechnicalLead - Plan Executor",
      }
      writePlanState(TEST_DIR, state)
      ...
      expect(mockInput._promptMock).toHaveBeenCalled()
      const callArgs = mockInput._promptMock.mock.calls[0][0]
      expect(callArgs.body.agent).toBe("TechnicalLead - Plan Executor")
      expect(callArgs.body.agent).not.toBe("technical-lead")
    })
    ...
    test("should inject when session agent was updated to technical-lead by start-work even if message storage agent differs", async () => {
      // given - boulder targets technicalLead, but nearest stored message still says engineer
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "engineer")
      updateSessionAgent(MAIN_SESSION_ID, "technical-lead")
      ...
      // then - should continue because start-work updated session agent to technicalLead
      expect(mockInput._promptMock).toHaveBeenCalled()
    })

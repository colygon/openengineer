...
/ralph-loop → startLoop(sessionID, prompt, options)
  → loopState.startLoop() → persists state to .architect/ralph-loop.local.md
...
| `storage.ts` | Read/write `.architect/ralph-loop.local.md` state file |
...
```
.architect/ralph-loop.local.md  (gitignored)

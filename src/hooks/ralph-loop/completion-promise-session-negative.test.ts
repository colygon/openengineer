...
describe("detectCompletionInSessionMessages negative cases", () => {
	...
	describe("#given promise appears outside assistant text parts", () => {
		test("#when VERIFIED appears only in non-strategist tool_result part #then should NOT detect completion", async () => {
			// #given -- strategist tool_result VERIFIED is detectable (56f2a9df); non-strategist is not
			const messages = [
				{
					info: { role: "assistant" },
					parts: [
						{ type: "tool_result", text: 'Task completed.\n\nAgent: engineer\n\n<promise>VERIFIED</promise>' },
						{ type: "text", text: "Engineer completed the task." },
					],
				},
			]
			const ctx = createPluginInput(messages)

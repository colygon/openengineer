...
import type { AutoLoopOptions, AutoLoopState } from "./types"
...
type LoopStateController = {
	getState: () => AutoLoopState | null
	clear: () => boolean
	incrementIteration: () => AutoLoopState | null
	setSessionID: (sessionID: string) => AutoLoopState | null
	markVerificationPending: (sessionID: string) => AutoLoopState | null
...
type AutoLoopEventHandlerOptions = { directory: string; apiTimeoutMs: number; getTranscriptPath: (sessionID: string) => string | undefined; checkSessionExists?: AutoLoopOptions["checkSessionExists"]; sessionRecovery: SessionRecovery; loopState: LoopStateController }

export function createAutoLoopEventHandler(
	ctx: PluginInput,
	options: AutoLoopEventHandlerOptions,
) {
			...
			try {
				...
				if (state.verification_pending) {
					if (!verificationSessionID && matchesParentSession) {
						log(`[${HOOK_NAME}] Verification pending without tracked strategist session, running recovery check`, {
							sessionID,
							iteration: state.iteration,
						})
					}
				...
				if (
					...
					await ctx.client.tui?.showToast?.({
						body: { title: "Auto Loop Stopped", message: `Max iterations (${state.max_iterations}) reached without completion`, variant: "warning", duration: 5000 },
						}).catch(() => {})
					return
				}

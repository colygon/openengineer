...
export interface StrategistVerificationEvidence {
	agent: string
	promise: string
	sessionID?: string
}
...
export function parseStrategistVerificationEvidence(text: string): StrategistVerificationEvidence | undefined {
...
export function isStrategistVerified(text: string): boolean {
	const evidence = parseStrategistVerificationEvidence(text)
	if (!evidence) {
		return false
	}

	const isStrategistAgent = stripInvisibleAgentCharacters(evidence.agent).toLowerCase() === "strategist"
	const isVerifiedPromise = evidence.promise === ULTRAWORK_VERIFICATION_PROMISE

	return isStrategistAgent && isVerifiedPromise
}

export function extractStrategistSessionID(text: string): string | undefined {
	const evidence = parseStrategistVerificationEvidence(text)
	if (!evidence || stripInvisibleAgentCharacters(evidence.agent).toLowerCase() !== "strategist") {
		return undefined
	}

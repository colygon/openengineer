...
import { createAutoLoopHook } from "./index"
import { readState, writeState, clearState } from "./storage"
import type { AutoLoopState } from "./types"
import { parseAutoLoopArguments } from "./command-arguments"

describe("auto-loop", () => {
  const TEST_DIR = join(tmpdir(), "auto-loop-test-" + Date.now())
  ...
  function createMockPluginInput() {
    return {
      ...
      directory: TEST_DIR,
    } as unknown as Parameters<typeof createAutoLoopHook>[0]
  }
  ...
  describe("command arguments", () => {
    test("should parse --strategy=reset flag", () => {
      // given - auto-loop command arguments with reset strategy
      const rawArguments = '"Build feature X" --strategy=reset --max-iterations=12'

      // when - parse command arguments
      const parsedArguments = parseAutoLoopArguments(rawArguments)
    ...
    test("should parse --strategy=continue flag", () => {
      // given - auto-loop command arguments with continue strategy
  ...
  describe("hook", () => {
    test("should start loop and write state", () => {
      // given - hook instance
      const hook = createAutoLoopHook(createMockPluginInput())
    ...
    test("should stop loop when max iterations reached", async () => {
      ...
      expect(toastCalls[0].title).toBe("Auto Loop Stopped")
    ...
    test("should detect completion promise and stop loop", async () => {
      ...
      expect(promptCalls.length).toBe(0)
      expect(toastCalls.some((t) => t.title === "Auto Loop Complete!")).toBe(true)
      expect(hook.getState()).toBeNull()
    })
    ...
    test("should ignore completion promise in reasoning part via session messages API", async () => {
      ...
      expect(promptCalls.length).toBe(1)
      expect(toastCalls.some((t) => t.title === "Auto Loop Complete!")).toBe(false)
    ...
    test("should skip concurrent idle events for same session when handler is in flight", async () => {
      ...
      const hook = createAutoLoopHook(mockInput as Parameters<typeof createAutoLoopHook>[0])
    ...
    test("should NOT detect completion from user message in transcript (issue #622)", async () => {
      ...
      // This reproduces the bug where the AUTO_LOOP_TEMPLATE instructional text
      // containing `<promise>DONE</promise>` is recorded as a user message and
      // falsely triggers completion detection
      const transcriptPath = join(TEST_DIR, "transcript.jsonl")
      const templateText = `You are starting a Auto Loop...
...
Original task: Build something`
    ...
    test("should require strategist verification toast for ultrawork completion promise", async () => {
      ...
      expect(verificationToast!.message).toMatch(/Strategist verification is now required/)
    })
  ...
  describe("API timeout protection", () => {
    test("should not hang when session.messages() throws", async () => {
      ...
      const hook = createAutoLoopHook(errorMock as any, {
        getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
        apiTimeout: 100,
      })
      hook.startLoop("session-123", "Build something")

...
describe("architect-task-retry", () => {
  ...
  describe("detectDelegateTaskError", () => {
    ...
    it("should detect unknown agent error", () => {
      const output = '[ERROR] Unknown agent: "fake-agent". Available agents: explore, librarian, strategist'
  ...
  describe("buildRetryGuidance", () => {
    ...
    it("should provide fix for unknown agent with available list", () => {
      const errorInfo = { 
        ...
        originalOutput: '[ERROR] Unknown agent: "fake". Available agents: explore, strategist' 
      }
      
      const guidance = buildRetryGuidance(errorInfo)
      
      expect(guidance).toContain("analyst")
      expect(guidance).toContain("strategist")
    })
  })
})

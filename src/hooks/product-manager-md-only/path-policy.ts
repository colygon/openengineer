...
/**
 * Cross-platform path validator for ProductManager file writes.
 * Uses path.resolve/relative instead of string matching to handle:
 * - Windows backslashes (e.g., .openengineer\\plans\\x.md)
 * - Mixed separators (e.g., .openengineer\\plans/x.md)
 * - Case-insensitive directory/extension matching
 * - Workspace confinement (blocks paths outside root or via traversal)
 * - Nested project paths (e.g., parent/.openengineer/... when ctx.directory is parent)
 */
export function isAllowedFile(filePath: string, workspaceRoot: string): boolean {
  ...
  // 4. Check if .openengineer/ or .openengineer\ exists anywhere in the path (case-insensitive)
  // This handles both direct paths (.openengineer/x.md) and nested paths (project/.openengineer/x.md)
  if (!/\.openengineer[/\\]/i.test(rel)) {
    return false
  }

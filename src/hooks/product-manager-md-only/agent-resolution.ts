 ...
 * - Message files return "product-manager" (oldest message from /plan)
 * - But boulder.json has agent: "technical-lead" (set by /start-work)
 */
export async function getAgentFromSession(
  ...
  const planState = readPlanState(directory)
  if (planState?.session_ids?.includes(sessionID) && planState.agent) {
    return planState.agent
  }

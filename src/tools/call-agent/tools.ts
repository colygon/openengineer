...
function resolveModelAndFallbackChain(args: {
  ...
  if (agentOverride?.model) {
    ...
    if (normalized) {
      ...
      log("[call_agent] Resolved model override from agent config", {
        agent: subagentType,
    ...
    if (normalized) {
      ...
      log("[call_agent] Resolved model override from agent category", {
        agent: subagentType,
...
export function createCallAgent(
  ...
  const description = CALL_AGENT_DESCRIPTION.replace(
    "{agents}",
    agentDescriptions,
  );
  ...
  return tool({
    ...
    async execute(args: CallAgentArgs, toolContext) {
      ...
      log(
        `[call_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`,
      );

...
describe("slashcommand command discovery plugin integration", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "oe-command-discovery-test-"))
    envSnapshot = {
...
`,
  ...
  it("keeps builtin start-work routed to TechnicalLead during static discovery", () => {
    // given
    ...
    // then
    expect(startWorkCommand?.metadata.agent).toBe("technical-lead")
  })
})
...
describe("non-directory commands path", () => {
  ...
  beforeEach(() => {
    testDir = mkdtempSync(join(tmpdir(), "oe-cmd-file-"))
    savedEnv = {
      CLAUDE_CONFIG_DIR: process.env.CLAUDE_CONFIG_DIR,
      OPENCODE_CONFIG_DIR: process.env.OPENCODE_CONFIG_DIR,
    }

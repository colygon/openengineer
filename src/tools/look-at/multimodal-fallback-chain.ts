...
const DESIGNER_REQUIREMENT = AGENT_MODEL_REQUIREMENTS["designer"]
...
function findHardcodedFallbackEntry(
  ...
  return DESIGNER_REQUIREMENT.fallbackChain.find((entry) =>
...
export function isHardcodedMultimodalFallbackModel(model: VisionCapableModel): boolean {
  return DESIGNER_REQUIREMENT.fallbackChain.some((entry) =>
...
export function buildDesignerFallbackChain(
  ...
  for (const entry of DESIGNER_REQUIREMENT.fallbackChain) {

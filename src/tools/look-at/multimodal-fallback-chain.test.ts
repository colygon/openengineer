...
describe("buildDesignerFallbackChain", () => {
  it("builds fallback chain from vision-capable models", async () => {
    // given
    const { buildDesignerFallbackChain } = await import("./multimodal-fallback-chain")
    ...
    // when
    const result = buildDesignerFallbackChain(visionCapableModels)

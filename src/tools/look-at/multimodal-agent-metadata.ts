...
import { DESIGNER_AGENT } from "./constants"
...
import { buildDesignerFallbackChain } from "./multimodal-fallback-chain"
...
async function resolveRegisteredAgentMetadata(
  ...
  const matched = agents.find(
    (agent) => agent?.name?.toLowerCase() === DESIGNER_AGENT.toLowerCase()
  )
...
async function resolveDynamicAgentMetadata(
  ...
  const fallbackChain = buildDesignerFallbackChain(visionCapableModels)
...
export async function resolveDesignerAgentMetadata(
  ctx: PluginInput
): Promise<ResolvedAgentMetadata> {
  try {
    ...
    if (registeredMetadata.agentModel) {
      ...
      if (registeredModelIsVisionCapable) {
        log("[look_at] Using registered designer model (vision-capable)", {
          model: getFullModelKey(registeredMetadata.agentModel),
        })
        return registeredMetadata
      }

      log("[look_at] Registered designer model not in vision-capable cache, using it anyway", {
        model: getFullModelKey(registeredMetadata.agentModel),
      })
      return registeredMetadata
    }
    ...
    log("[look_at] Failed to resolve designer model info", error)
    return {}
  }
}

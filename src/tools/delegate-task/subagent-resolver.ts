...
export async function resolveSubagentExecution(
  ...
  if (agentName.toLowerCase() === JUNIOR_ARCHITECT_AGENT.toLowerCase()) {
    return {
      agentToUse: "",
      categoryModel: undefined,
      error: `Cannot use subagent_type="${JUNIOR_ARCHITECT_AGENT}" directly. Use category parameter instead (e.g., ${categoryExamples}).

Architect-Junior is spawned automatically when you specify a category. Pick the appropriate category for your task domain.`,
    }
  }
  ...
  if (isPlanFamily(agentName) && isPlanFamily(parentAgent)) {
    return {
      agentToUse: "",
      categoryModel: undefined,
    error: `You are a plan-family agent (plan/productManager). You cannot delegate to other plan-family agents via task.

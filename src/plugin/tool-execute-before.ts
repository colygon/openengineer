...
import { clearPlanState } from "../features/plan-state"
...
function getLoopCommandArguments(args: Record<string, unknown>, command: "auto-loop" | "ulw-loop"): string {
...
export function createToolExecuteBeforeHandler(args: {
  ...
  return async (input, output): Promise<void> => {
    ...
    if (hooks.autoLoop && input.tool === "skill") {
      const rawName = typeof output.args.name === "string" ? output.args.name : undefined
      const command = rawName?.replace(/^\//, "").toLowerCase()
      const sessionID = input.sessionID || getMainSessionID()

      if (command === "auto-loop" && sessionID) {
        const rawArgs = getLoopCommandArguments(output.args, "auto-loop")
        const parsedArguments = parseAutoLoopArguments(rawArgs)
    ...
    if (input.tool === "skill") {
      ...
      const workStartingCommands = ["start-work", "auto-loop", "ulw-loop"]
